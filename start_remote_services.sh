...
CONFIG_TEMPLATE_FILE="$REPO_DIR/vesper.conf.example"
...
pre_flight_checks() {
    ...
    # Auto-create vesper.conf from template if it's missing.
    if [ ! -f "$CONFIG_FILE" ]; then
        echo "🤔 Config file not found. Attempting to create from template..."
        if [ ! -f "$CONFIG_TEMPLATE_FILE" ]; then
            echo "❌ CRITICAL: Config template not found at $CONFIG_TEMPLATE_FILE." >&2
            all_checks_passed=false
        else
            cp "$CONFIG_TEMPLATE_FILE" "$CONFIG_FILE"
            echo "✅ Created '$CONFIG_FILE'. Please review it for correct settings."
        fi
    fi
    ...
    # Source the config file *after* it has been potentially created.
    if [ -f "$CONFIG_FILE" ]; then
        source "$CONFIG_FILE"
        MODEL_PATH_CHECK="${VESPER_MODEL_PATH:-$MODEL_PATH}"
        if [ -z "$MODEL_PATH_CHECK" ] || [ "$MODEL_PATH_CHECK" == "/workspace/runpod-babylegs/models/your_model_name.gguf" ]; then
             echo "⚠️ WARNING: Model path is not set or is set to the default example. Please update '$CONFIG_FILE'."
        elif [ ! -f "$MODEL_PATH_CHECK" ]; then
            echo "❌ CRITICAL: Model file not found at '$MODEL_PATH_CHECK'." >&2; all_checks_passed=false
        fi
    else
        echo "❌ CRITICAL: Config file '$CONFIG_FILE' is missing and could not be created." >&2
        all_checks_passed=false
    fi
